...
<div 
    ...
    tabindex="0"
    class="bg-white p-4 rounded-xl shadow-sm border border-gray-200/80 cursor-grab active:cursor-grabbing hover:shadow-md hover:border-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all space-y-3"
    role="button"
...
>
    ...
    <div>
        <h4 class="font-bold text-gray-900 text-sm leading-snug tracking-tight">{{ $task->title }}</h4>
        @if($task->description)
            <p class="text-xs text-gray-500 mt-1 line-clamp-2 leading-relaxed">{{ $task->description }}</p>
        @endif
    </div>
    ...
    <div class="flex justify-between items-center pt-2 border-t border-gray-50 text-[11px] text-gray-400">
        ...
        <!-- Due Date -->
        @if($task->due_date)
            <span class="flex items-center space-x-1 font-semibold text-gray-500" title="Due Date">
                <svg class="h-3.5 w-3.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
